...
import subprocess
import utils
utils.check_env.default_config()
...
preprompt_text = (
    ...
    f"Do not include any typographical mark in respond. You can use PyAutoGUI for controlling mouse and keyboard.Remember that whenever you use spotlight on mac you must use pyautogui.hotkey('command','space', interval=0.1) that interval must should be set."
...
def threat_intelligence(python_code):
    threat_result = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "user", "content": threat_preprompt_text + python_code}]
    )
    threat_score = int(threat_result.choices[0].message.content)
    if threat_score > config["Threat_Barrier"]:
        # engine.say(
        #     "This is an Avengers Level Threat. Do you want to proceed?")
        # engine.runAndWait()
...
while True:
    ...
    python_code = completion.choices[0].message.content.replace(
        'super', 'win')

    if threat_intelligence(python_code):
        execute_command(python_code)
    else:
        print("An error occurred in threat_intelligence")
